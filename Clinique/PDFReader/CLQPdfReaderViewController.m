...
- (void)updateToolbarBookmarkIcon
{
    ...
    NSLog(@"Book marks :%@ and current page :%d",[CacheManager defaultManager].currentCache.bookMarks,currentPage);
...
-(IBAction)bookMarkClicked:(id)sender{
    @try {
       ...
       /* if(self.currentBookmark == nil){
        ...
        if ([bookMarks containsObject:@([page intValue])]) // Remove bookmark
        {
            [self.bookMarkbutton setSelected:NO];
            [bookMarks removeObject:@([page intValue])];
            type = DeleteBookMark;
            //Comment for Online capability
        ...
        else // Add the bookmarked page index to the bookmarks set
        {
            ...
            [bookMarks addObject:@([page intValue])];
            type = InsertBookMark;
            //Comment for Online capability
            /*if(![addedBookmarks containsObject:page]){
        ...
        if([page intValue] > 0 && [page intValue] <= [self.document.pageCount intValue]){
            ...
            [[PDFRequestManager defaultManager]updateBookMark:[CacheManager defaultManager].currentCache.courseModuleId forPage:[page intValue] forServiceType:type withCompletion:^(PDFResponse *response, NSError *error){
            }];
    ...
    @try {
       ...
       /* self.commentButton.userInteractionEnabled = YES;
        ...
        if(state == CommentStateAdd){
            ...
            [CacheManager defaultManager].currentCache.comments =comment;
            [[CacheManager defaultManager]saveCache];
            [[PDFRequestManager defaultManager]updateComment:[CacheManager defaultManager].currentCache.courseModuleId forService:0 withComment:comment withCompletion:^(PDFResponse *response,NSError *error){}];
