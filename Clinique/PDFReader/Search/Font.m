...
- (unichar)mappedUnicodeValue:(CharacterCode)character
{
    ...
    return (value == (CharacterCode)NSNotFound) ? value : [self encodedUnicodeValue:character];
}
...
- (unichar)encodedUnicodeValue:(CharacterCode)character
{
    ...
    NSString *string = [NSString stringWithCString:str encoding:stringEncoding];
    if(string.length > 0)
...
**/
- (NSString *)stringWithPDFString:(const unsigned char *)bytes length:(unsigned int)length
{
    NSMutableString *string = [NSMutableString string];
    for (int i = 0; i < length; i++)
    {
        CharacterCode code = bytes[i];
...
**/
- (NSString *)unicodeWithPDFString:(const unsigned char *)bytes length:(unsigned int)length
{
	if (self.toUnicode)
	{
		NSMutableString *unicodeString = [NSMutableString string];
        ...
        return [self stringWithPDFString:bytes length:length];
    }
}
