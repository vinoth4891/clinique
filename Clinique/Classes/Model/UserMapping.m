...
+(void)insertUserMapping:(NSDictionary *)dict{
    ...
    [database executeUpdate:@"INSERT INTO UserMappings (userId,refrenceId,mappingType) VALUES (?, ?, ?)",userMapping.userId ,userMapping.refrenceId,userMapping.mappingType , nil];
...
+(void)updateUserMapping:(NSDictionary *)dict{
    ...
    FMDatabase *database = [FMDatabase databaseWithPath:[[CLQDataBaseManager dataBaseManager]getDBPath]];
    [database open];
    [database executeUpdate:@"UPDATE UserMappings set userId= ?, refrenceId= ?,mappingType= ? where userId= ? and refrenceId = ? and mappingType =  ?",category.userId,category.refrenceId,category.mappingType,category.userId,category.refrenceId,category.mappingType, nil];
    [database close];
}
