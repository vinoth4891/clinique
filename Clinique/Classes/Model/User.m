...
+(id)getUserDetails:(NSDictionary *)dict{
    ...
    [database open];
    FMResultSet *results = [database executeQuery:@"SELECT * from Users where TRIM(upper(username)) = TRIM(upper(?)) and password = ? and firstTime = ?",dict[kUserName],dict[kPassword], @"N"];
...
+(id)objectFromUser:(id)object{
    ...
    if([object isKindOfClass:[FMResultSet class]]){
        ...
        user.userName = [User getUserName:[results stringForColumn:kUserName]];
        ...
        if(dict[kUserName] != [NSNull null]){
            user.userName = [User getUserName:dict[kUserName]];
        }
...
+(NSString *)getUserName:(NSString *)userName{
    NSString *str = [userName stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
