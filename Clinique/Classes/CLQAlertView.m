...
@interface CLQAlertOverlayWindow : UIWindow
{
}
@property (nonatomic,retain) UIWindow* oldKeyWindow;
@end

@implementation  CLQAlertOverlayWindow
...
@interface CLQAlertView (private)
...
{
	CLQAlertView* av = [self.view.subviews lastObject];
	if (!av || ![av isKindOfClass:[CLQAlertView class]])
		return;
...
- (id) initWithTitle: (NSString *) t message: (NSString *) m delegate: (id) d cancelButtonTitle: (NSString *) cancelButtonTitle otherButtonTitles: (NSString *) otherButtonTitles, ...
{
	if ( (self = [super init] ) ) // will call into initWithFrame, thus TSAlertView_commonInit is called
	{
		...
		{
			[self addButtonWithTitle: cancelButtonTitle withBackgroundImage:nil];
		...
		if ( nil != otherButtonTitles )
		{
            ...
            [self addButtonWithTitle: otherButtonTitles withBackgroundImage:nil];
			...
			while ( nil != ( arg = va_arg( args, id ) ) ) 
			{
				...
				[self addButtonWithTitle: (NSString*)arg withBackgroundImage:nil];
			}
		}
	}
...
- (void) TSAlertView_commonInit
{
	...
	// defaults:
	style = CLQAlertViewStyleNormal;
	self.width = 0; // set to default
	self.maxHeight = 0; // set to default
	buttonLayout = CLQAlertViewButtonLayoutNormal;
	cancelButtonIndex = -1;
...
- (void) setStyle:(CLQAlertViewStyle)newStyle
{
	if ( style != newStyle )
	{
		style = newStyle;
		
		if ( style == CLQAlertViewStyleInput )
		{
...
- (UILabel*) messageLabel
{
	if ( _messageLabel == nil )
	{
		...
		_messageLabel.font = [UIFont systemFontOfSize: 60];
...
- (NSInteger) addButtonWithTitle: (NSString *) t withBackgroundImage:(UIImage *)image
{
	UIButton* b = [UIButton buttonWithType: UIButtonTypeCustom];
    [b.titleLabel setFont:[UIFont fontWithName:@"Helvetica" size:14.0f]];
...
{
	if ( self.style == CLQAlertViewStyleInput && [self.inputTextField isFirstResponder] )
	{
...
- (void) show
{
	...
	CLQAlertOverlayWindow* ow = [[CLQAlertOverlayWindow alloc] initWithFrame: [UIScreen mainScreen].bounds];
	ow.alpha = 0.0;
	...
	if ( self.style == CLQAlertViewStyleInput )
	{
		[self layoutSubviews];
...
- (CGSize) recalcSizeAndLayout: (BOOL) layout
{
	BOOL	stacked = !(self.buttonLayout == CLQAlertViewButtonLayoutNormal && [self.buttons count] == 2 );
	...
	CGFloat inputRowHeight = self.style == CLQAlertViewStyleInput ? inputTextFieldSize.height + kTSAlertView_RowMargin : 0;
	...
	if ( layout )
	{
		...
		if ( stacked )
		{
			...
			{
				b.frame = CGRectMake( kTSAlertView_LeftMargin, y+10, buttonWidth, buttonHeight );
				[self addSubview: b];
				y += buttonHeight + kTSAlertView_RowMargin+10;
			}
		}
		else
		{
...
- (CGSize) inputTextFieldSize
{
	if ( self.style == CLQAlertViewStyleNormal)
		return CGSizeZero;
