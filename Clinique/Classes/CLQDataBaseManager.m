...
-(id)getQuizDictionary:(NSDictionary *)dict forAsset:(Asset *)asset{
    @try {
        ...
        NSString *referenceId = [NSString stringWithFormat:@"%@",asset.referenceId];
        
        UserMapping *usermapping = [UserMapping getUserMapping:@{kuserId : userId, kRefrenceId : referenceId, kUserMappingType : kUser_Mapping_Group_Module}];
        ...
        for (NSDictionary *quizlist in array)
        {
            ...
            if(quizListDict[kAttempts] != [NSNull null] && quizListDict[kAttempts])
            {
                //NSArray *attempts = quizListDict[kAttempts];
                if(usermapping.attemptsJson != nil){
                   // NSData *data = [localStoarge.json dataUsingEncoding:NSUTF8StringEncoding];
                    NSDictionary *usermappingAttempts = [NSJSONSerialization JSONObjectWithData:usermapping.attemptsJson options:kNilOptions error:nil];
                    [quizListDict removeObjectForKey:kAttempts];
                    quizListDict[kAttempts] =usermappingAttempts[Key_Quiz][kAttempts];
        ...
        for (NSDictionary *quizInfo in quizInfos)
        {
            ...
            if(usermapping != nil)
            {
                ...
                NSDictionary *userMappingJson = [NSJSONSerialization JSONObjectWithData:usermapping.attemptsJson options:kNilOptions error:nil];
                
                if(quizInfoDict[Key_AttemptedCount]){
                    [quizInfoDict removeObjectForKey:Key_AttemptedCount];
                }
                
                NSString *attemptedCount  =[NSString stringWithFormat:@"%@",userMappingJson[Key_Quiz][Key_AttemptedCount]];
                quizInfoDict[Key_AttemptedCount] = @([attemptedCount intValue]);
                
                if(quizInfoDict[Key_Anyfinished]){
                    [quizInfoDict removeObjectForKey:Key_Anyfinished];
                }
                NSString *anyFinished  =[NSString stringWithFormat:@"%@",userMappingJson[Key_Quiz][Key_Anyfinished]];
                quizInfoDict[Key_Anyfinished] = @([anyFinished intValue]);
                ...
                if(quizInfoDict[kNewAttempts]){
                 ...
                 quizInfoDict[kNewAttempts]  = userMappingJson[Key_Quiz][kNewAttempts];
                 quizDict[Key_Quiz_Info]  = quizInfoDict;
        ...
        if(quizDict[Key_Quiz_Info]){
            // [dict[kuserId] intValue]), @([dict[kRefrenceId] intValue]), dict[kUserMappingType]
