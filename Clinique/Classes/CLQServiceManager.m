...
+(void)saveSyncDataInDatabase:(NSDictionary *)dict withParams:(NSDictionary *)params forServiceType:(ServiceType)serviceType withCompletion:(void(^)(BOOL completed))completion{
    ...
    {
        NSArray *quizSyncs = dict[Key_Quiz_Sync];
        for (NSDictionary *quizSync in quizSyncs) {
            [UserMapping saveUserMapping:@{kuserId : [CLQDataBaseManager dataBaseManager].currentUser.userId, kRefrenceId : quizSync[kId], kUserMappingType :kUser_Mapping_Group_Module,kAttempts : quizSync}];
             [QuizLocalStorage saveLocalStorage:quizSync];
        }
...
+(id)getQuizRequestBodyForQuizes:(QuizLocalStorage *)localStoarge{
    ...
    if([object isKindOfClass:[NSDictionary class]])
    {
        ...
        if(quizDict[Key_QuizList] != [ NSNull null] && quizDict[Key_QuizList])
        {
            ...
            if(quizDict[Key_Questions])
                [quizDict removeObjectForKey:Key_Questions];
...
-(void)connection:(NSURLConnection *)connection didReceiveResponse:(NSURLResponse *)response{
    ...
    if([httpResponse statusCode]== 200){
         NSString *filePath =   [CLQHelper createLocalAssetPath:self.currentDownloadingAsset];
        NSLog(@"created filePath : %@", filePath);
        self.currentDownloadingAsset.assetSize= [NSString stringWithFormat:@"%lld",response.expectedContentLength]  ;
        [Asset saveAssetSize:self.currentDownloadingAsset];
