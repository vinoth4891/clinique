    ...
    Clazz.com.components.widget.favorites.js.Favorites = Clazz.extend(Clazz.WidgetWithTemplate, {
        ...
        bindUI : function(){
            ...
            jQuery('.ifram_cls_btn').die().live('click',function(){
                ...
                self.footerIcons(false);
                loadAllLanguages();
                if( self.globalConfig.application.offLine ){
...
//                });
            ...
            if(isDevice() && isPhoneGap()){
                ...
                if( /Android/i.test(navigator.userAgent) ) {
                      ...
                      cordova.exec(
                                   function (args) {},
                                   function (args) {},
                                   ''+((self.globalConfig.application.offLine)?'FileOpener':'FileOpenerOnline')+'', '' +((file_Type === 'pdf')?'openFile':'openVideoFile')+ '', [((file_Type === 'pdf')?androidData:self.plugINURl)]);
                      if( file_Type === 'pdf' ){
                       jQuery("#showlistFav").show();
                      }
      ...
      //     var breadFourTitle = (CCbreadCrumb != 'CC:' ? quizTitle:quizTitle.substr(3));
        ...
        getNotesData: function(){
            ...
            self.ajaxReq(self,serviceUrl,data,function(resp){
                var res = resp;//JSON.parse(resp);
                 if( isDevice() && !res.error && self.globalConfig.application.offLine ){
                    res.error = false;
                 }
        ...
        ajaxReq:function(self,serviceUrl,data,succ,fail){
             if( isDevice() && self.globalConfig.application.offLine){
                  cordova.exec(
                               function(result) { succ(JSON.parse(result)); },
                               function(result) {},'OfflineServicePlugin', 'get_course_resource_comments', [data]);
             }else{
				jQuery.ajax({
					url: serviceUrl,
					data: data,
        ...
        loadFileinWebFav: function(self,courseItemData) {
            var self=this,filePath = courseItemData.fileURL, language, userID, androidData={},emptyCSV=false;
            if (!($.browser.msie && parseInt($.browser.version, 10) === 7)) {
                language = window.localStorage.getItem("language");
                userID = JSON.parse(window.localStorage.getItem("USER")).id;
            } else {
            ...
            if(isDevice()  && isPhoneGap()){
                if( /Android/i.test(navigator.userAgent) ) {
                  ...
                  cordova.exec(
                               function (args) {},
                               function (args) {},
                               ''+((self.globalConfig.application.offLine)?'FileOpener':'FileOpenerOnline')+'', 'openCSVFile', [androidData,self.mod_Ids]);
                 if( emptyCSV ){
                    self.mod_Ids=[];
                 }
