...
if (!($.browser.msie && parseInt($.browser.version, 10) === 7)) {
    locStrgpdfUrl = window.localStorage.pdfurl;
} else {
    locStrgpdfUrl = $.jStorage.get("pdfurl");
}
var DEFAULT_URL = locStrgpdfUrl;
...
var FindStates = {
  ...
  PDFJS.imageResourcesPath = '../lib/pdf/images/';
  PDFJS.workerSrc = '../lib/pdf/pdf.worker.js';
  PDFJS.cMapUrl = '../lib/pdf/cmaps/';
...
Preferences._writeToStorage = function (prefObj) {
  return new Promise(function (resolve) {    
    if (!($.browser.msie && parseInt($.browser.version, 10) === 7)) {
        ...
        $.jStorage.set('pdfjs.preferences', JSON.stringify(prefObj));
    }
    resolve();
  });
};
  ...
  return new Promise(function (resolve) {
    var readPrefs;
        if (!($.browser.msie && parseInt($.browser.version, 10) === 7)) {
            ...
            readPrefs = JSON.parse($.jStorage.get('pdfjs.preferences'));
        }    
    resolve(readPrefs);
  });
};
...
var ViewHistory = (function ViewHistoryClosure() {
  function ViewHistory(fingerprint) {
    ...
    var localStrgDbName;
        if (!($.browser.msie && parseInt($.browser.version, 10) === 7)) {
            localStrgDbName = localStorage.getItem('database')
        } else {
            localStrgDbName = $.jStorage.get('database');
        }
    resolvePromise(localStrgDbName);
  }
    ...
    set: function ViewHistory_set(name, val) {
      ...
      if (!($.browser.msie && parseInt($.browser.version, 10) === 7)) {
        localStorage.setItem('database', database);
            ...
            $.jStorage.set('database', database);
      }
    },
  ...
  updateUIState: function(state, previous) {
    ...
    switch (state) {
      ...
      case FindStates.FIND_NOTFOUND:
        findMsg = mozL10n.get('find_not_found', null, 'Search Keyword Not Found');
        notFound = true;
        break;
...
var SecondaryToolbar = {
  ...
  downloadClick: function secondaryToolbarDownloadClick(evt) {
     ...
     if( browserName[1] == "Safari" && downLoad.getAttribute('data-safari') == 'true' ){
...
function webViewerInitialized() {
  ...
  if (!PDFView.supportsFullscreen) {
    document.getElementById('presentationMode').classList.add('hiddenn');
    document.getElementById('secondaryPresentationMode').
...
function updateViewarea() {
  ...
  //document.getElementById('viewBookmarkLocale').href = "javascript:void(0);";
  document.getElementById('secondaryViewBookmark').href = "javascript:void(0);";
...
window.addEventListener('pagechange', function pagechange(evt) {
  ...
  if (PDFView.previousPageNumber !== page) {
    ...
    var book_mark = document.getElementById('viewBookmarkLocale');   
    var thumbnail = document.getElementById('thumbnailContainer' + page);
    var thumnailChild = thumbnail.getElementsByClassName('bookmarked'); 
    if(thumnailChild.length){  
       book_mark.classList.remove('bookmark');
       book_mark.classList.add('bookmarked');    
       book_mark.setAttribute ('data-bookmarked','true');    
    }else{
       book_mark.classList.add('bookmark');
       book_mark.classList.remove('bookmarked');
       book_mark.setAttribute ('data-bookmarked','false');     
    }
