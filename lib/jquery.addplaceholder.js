		...
		function inputPlacehold(el, ph) {
			...
			el.focusout(function(){
				if ( valueEmpty(el.val()) ) {
					var getPh = this.getAttribute('placeholder');
					el.val(getPh);
					el.addClass(settings['class']);
				}
			});
			...
			if(ph == "password"){
				var span = $('<span/>',{
					...
					css: {
						...
						color:		'#95999a',			// needs adjustment due to extra "natural" padding of input fields
						paddingLeft:parseFloat(el.css('paddingLeft'))-10+'px'	// ditto as above  
					}
				}).attr('id',"passwordSpan").insertAfter(el).css('text-align','left').css('text-indent','1px');
			}
			else{
				var span = $('<span/>',{
					'class': el.attr('class')+' '+settings['class'],	// 'inherit' class from the input field + the placeholder class
				...
				}).attr('id',"passwordSpan1").insertAfter(el).css('text-align','left').css('text-indent','1px');
			}
