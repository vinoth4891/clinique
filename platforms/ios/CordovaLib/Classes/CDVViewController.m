...
-(void)iosPlugin:(id)progressJson{
    
    if(progressJson == nil){
        // [self.webView stringByEvaluatingJavaScriptFromString:@"progressInitiate()"];
        
    }
    else{
        [self.webView stringByEvaluatingJavaScriptFromString:[NSString stringWithFormat:@"sendProgress(%@)",progressJson]];
    }
    
}

-(void)javascriptNetworkPlugin:(BOOL)isNetWorkAvailable{
    
    [self.webView stringByEvaluatingJavaScriptFromString:[NSString stringWithFormat:@"iosNetworkStatus(%hhd)",isNetWorkAvailable]];
}

-(void)syncExceptionPluginCalled:(NSString *)error{
    
    [self.webView stringByEvaluatingJavaScriptFromString:[NSString stringWithFormat:@"progressFailiure(%@)",error]];
}

-(void)progressStopPlugin{
    [self.webView stringByEvaluatingJavaScriptFromString:@"noContent_HideProgres()"];
}

-(void)showOrHideBackButton:(BOOL)isShoworHide{
    [self.webView stringByEvaluatingJavaScriptFromString:[NSString stringWithFormat:@"BObuttonactive(%hhd)",isShoworHide]];
}
