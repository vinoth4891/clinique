...
if (!empty($user)) {
    ...
    app_complete_user_login($user);
    $forcePasswordChangesql = "SELECT up.userid
              FROM {user_preferences} up
             WHERE up.userid = ? AND up.name = ?";
    $forcePasswordChange = $DB->get_records_sql($forcePasswordChangesql, array($user->id, 'auth_forcepasswordchange'));
...
function app_complete_user_login($user) {
    global $CFG, $USER;
