...
if ($mform->is_cancelled()) {
    ...
    if (($qtypeobj->finished_edit_wizard($fromform)) || $movecontext) {
        if ($inpopup) {
			...
			/**
			 * Modifying the question did not get reflected in delta sync 
			 * so updating quiz timemodified date whenever the question / answer associated with this quiz get modified.
			 */
			$record = new stdClass();
			$record->id = $cm->instance;
			$record->timemodified = time();
			$ret = $DB->update_record('quiz', $record, $bulk=false);
            redirect($returnurl);
        }
