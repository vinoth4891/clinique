...
function set_user_preference($name, $value, $user = null) {
    ...
    $value = (string)$value;
    if (textlib::strlen($value) > 4294967295) { //value column maximum length is 1333 characters
...
function setnew_password_and_mail($user) {
    ...
    return email_to_user($user, $supportuser, $subject, $message); // An email will be triggered to self-registered user's with out admin approval.
    //return email_to_user($supportuser, $supportuser, $subject, $message); // An email will be received by admin to validate/approve user's account.
