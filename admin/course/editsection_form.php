...
class editsection_form extends moodleform {
    ...
    function definition() {
		...
		$mform->registerRule('checksectionposition', 'callback', 'check_section_position', 'editsection_form');
		$mform->addElement('text', 'section_position', get_string('sectionposition'), array('size'=>'30'));
		$mform->addRule('section_position', get_string('invalidsectionposition'), 'checksectionposition');
        $mform->setType('section_position', PARAM_TEXT);
		
		$mform->addElement('checkbox', 'setlogo', get_string('sectionlogo'));
        $mform->setDefault('setlogo', 0);
	...
	public function check_section_position($section_position) {
		if($section_position >=0 && $section_position <=6)
			return true;
