...
class core_course_external extends external_api {
     ...
     * @since Moodle 2.2
     */
    public static function get_course_contents($courseid, $options = array(), $from) {
        global $CFG, $DB;
        ...
        try {
	    //@todo Temporarily disabling it to workaround "cannot execute functions in the course context..." issue
        ...
        if ($canupdatecourse or $course->visible
            ...
            $courseformatoptions = course_get_format($course)->get_format_options(); // To get numsections of course - Kesavamoorthy - 13/03/2013
            ...
            foreach ($sections as $key => $section) {
                ...
                $sectionvalues['section_position'] = $section->section_position;
                $sectionvalues['logo'] = $section->setlogo;                
                $sectionvalues['numsections'] = $courseformatoptions['numsections'];
                list($sectionvalues['summary'], $sectionvalues['summaryformat']) =
                        external_format_text($section->summary, $section->summaryformat,
                                $context->id, 'course', 'section', $section->id);
				 $sectionvalues['tile_content'] = self::getTextBetweenTags(html_entity_decode($sectionvalues['summary']), 'tile');
				 $sectionvalues['heading'] = self::getTextBetweenTags(html_entity_decode($$sectionvalues['summary']), 'heading');				
                                 $sectionvalues['timemodified'] = self::getmodifytime($secid);
                ...
                foreach ($modinfo->sections[$section->section] as $cmid) { //matching /course/lib.php:print_section() logic
					...
					if( ( ! empty($course->viewtype) && empty($cm->visible)) || $cm->modname === 'glossary')
						continue;

                    // stop here if the module is not visible to the user
                    /* if (!$cm->uservisible) { // Commented to view the conditional data items(Greyed out) - Kesavamoorthy - 12/03/2013.
                        continue;
                    } */
                    ...
                    if (!empty($cm->showdescription)) {
						...
						preg_match('/<img(.*)src(.*)=(.*)"(.*)"/U', html_entity_decode($module['description']), $imagesrc);
						 $module['hiddenimage'] = $imagesrc[4];
						 $module['tile_content'] = self::getTextBetweenTags(html_entity_decode($module['description']), 'tile');
						 $module['heading'] = self::getTextBetweenTags(html_entity_decode($module['description']), 'heading');	
                    }
                    ...
                    //Added to view the conditional items with the object showavailability - Kesavamoorthy - 12/03/2013.
                    if (!$cm->uservisible) {
                        $module['showavailability'] = 2;
                    }
                    else{
                        $module['showavailability'] =  $cm->showavailability;
                    }
                    ...
                    if (function_exists($getcontentfunction)) {
                        if ($contents = $getcontentfunction($cm, $baseurl)) {
                            ...
                            /* PDF Page number - Saravanan - 27/02/2012 */
                            $arraycnt = count($module['contents']);
                            $admins = get_admins();
                            $currentAdmin = end($admins);
                            $admintokensql = "SELECT et.token FROM {external_tokens} et WHERE et.userid = ?";
                            ...
                            for($j=0;$j<$arraycnt; $j++) {
                                $pdffile = strstr($module['contents'][$j]['filename'], '.pdf', true);
                                if($pdffile != '') {
                                    $fileurl = $module['contents'][$j]['fileurl'].'&token='.$keys[0];
                                    $pdftext = file_get_contents($fileurl);
                                    $num = preg_match_all("/\/Page\W/", $pdftext, $dummy);
                                    $module['contents'][$j]['pageno'] = $num;
                                } else {
                                    $module['contents'][$j]['pageno'] = 0;
                                }
                           }
                           /* PDF Page number - Saravanan - 27/02/2012 */
                        }
    ...
    public static function getmodifytime($sid) {
        global $DB;        
        $sectionRecord = $DB->get_record('course_sections',array(
					'id' => $sid
				));
        $tm='';
        if(!empty($sectionRecord)){
          $tmodify = $sectionRecord->timemodified;
          $tm = strtotime($tmodify);
        }
        return $tm;        
    }
        ...
        return new external_multiple_structure(
            new external_single_structure(
                array(
					...
					'section_position' => new external_value(PARAM_INT, 'number of sections'),
					'logo' => new external_value(PARAM_INT, 'Logo'),
                    ...
                    'tile_content' => new external_value(PARAM_RAW, 'Tile Content'),					
                    'modules' => new external_multiple_structure(
                            new external_single_structure(
                                array(
                                    ...
                                    'tile_content' => new external_value(PARAM_RAW, 'tile content', VALUE_OPTIONAL),
                                    'heading' => new external_value(PARAM_RAW, 'heading', VALUE_OPTIONAL),
                                    'hiddenimage' => new external_value(PARAM_RAW, 'hidden image for aae activity', VALUE_OPTIONAL),
                                    ...
                                    'favorite' => new external_value(PARAM_TEXT, 'Content Favorite status'), /* Favorite - Jenson - 08/03/2013 */
                                    'showavailability' => new external_value(PARAM_INT, 'showavailability'), /* Course Content Items - showavailability - Kesavamoorthy - 12/03/2013 */
                                    'contents' => new external_multiple_structure(
                                          new external_single_structure(
                                              array(
                                                  ...
                                                  'pageno' => new external_value(PARAM_INT, 'Content PDF page number'), /* PDF Page number - Saravanan - 27/02/2012*/
                                              )
                                          ), VALUE_DEFAULT, array()
                                      )
                                )
