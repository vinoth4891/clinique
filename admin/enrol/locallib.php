...
class course_enrolment_manager {
    ...
    public function get_potential_users($enrolid, $search='', $searchanywhere=false, $page=0, $perpage=25) {
		...
		$join = '';
		if($this->currentuserrolename == 'subadmin') {			
			/*$user_region = $DB->get_records_menu('region_user', array('userid' => $USER->id) , $sort='', $fields='id,region');
			if($user_region) {
				$regions_list = implode("','", array_values($user_region));
				$regionId = array_values($DB->get_records_sql("SELECT id FROM {user_info_field} WHERE shortname='region'"));
				if($wherecondition) {
					$wherecondition .= " AND ";
				}
				$enrolledusercourses = enrol_get_users_courses($USER->id, 2, '');
				foreach ($enrolledusercourses as $kk => $user_en_course_list) {
					var_dump($user_en_course_list);
				}
				$wherecondition .= " ud.data IN ('".$regions_list."') AND ud.fieldid ='".$regionId[0]->id."'";
				$join = ' JOIN {user_info_data} ud ON u.id = ud.userid ';
			}*/
			...
			foreach($get_all_cat as $get_k => $get_v) {
				$enrolledusercourses = enrol_get_users_courses($USER->id, $get_v->id, '');
				foreach ($enrolledusercourses as $kk => $user_en_course_list) {
					$courses_id[] = $user_en_course_list->id;
				}
			}
			$course_id_lists = implode("','", $courses_id);
			$enrol_ids = $DB->get_records_sql("SELECT * FROM {enrol} WHERE courseid IN ('".$course_id_lists."')");
			
			if($enrol_ids) {			
				foreach($enrol_ids as $kkey => $enroll_id) {
					if($enrolid != $enroll_id->id) {
						$enroll_id_list[] = $enroll_id->id;
					}
				}
				$enroll_id_list_str = implode("','", $enroll_id_list);
				if($wherecondition) {
					$wherecondition .= " AND ";
				}
				$wherecondition .= " ue.enrolid IN ('".$enroll_id_list_str."')";
				$wherecondition .= "AND ue.userid NOT IN (SELECT userid FROM mdl_user_enrolments WHERE enrolid = ".$enrolid.")";
				//var_dump($enroll_id_list);				
			}
		}
