...
class core_enrol_external extends external_api {
    ...
    public static function get_users_courses_parameters() {
        return new external_function_parameters(
            array(
                ...
                'cattype' => new external_value(PARAM_RAW, 'category type')
            )
        );
    }
     ...
     */
    public static function get_users_courses($userid, $categoryid, $cattype=NULL) {

        global $USER, $DB;

        // Do basic automatic PARAM checks on incoming data, using params description
        // If any problems are found then exceptions are thrown with helpful error messages
        $params = self::validate_parameters(self::get_users_courses_subcat_parameters(), array('userid'=>$userid,'categoryid'=>$categoryid,'cattype'=>$cattype));

        $list_of_subcategories = $DB->get_records('course_categories', array('parent' => $params['categoryid']));

		$courses = enrol_get_users_courses($params['userid'],$params['categoryid'],$params['cattype'], true, 'id, shortname, fullname, idnumber, visible, viewtype, coursepreview');
		if($list_of_subcategories) {
			foreach($list_of_subcategories as $kSubCat => $vSubCat) {
				$subcategory = $vSubCat->id;
				$courses = array_merge($courses,enrol_get_users_courses($params['userid'],$subcategory,$params['cattype'], true, 'id, shortname, fullname, idnumber, visible, viewtype, coursepreview'));
        ...
        foreach ($courses as $course) {
            ...
            /**
             * Logic to find the type of course as based on startdate - 21-02-2013 - Course Status(Coming Soon)
             */
            
           $coursestartdate = $course->startdate;
           $today = time();
           $aweekago = strtotime('-30 days');
            if($coursestartdate > $today){
                $coursestatus = 'Coming Soon';
            }
            //if($aweekago <= $coursestartdate && $coursestartdate)
                //$paymentDate >= $contractDateBegin && $paymentDate <= $contractDateEnd
            else if($coursestartdate >= $aweekago && $coursestartdate <= $today){
                $coursestatus = 'New';
            }
            else{
                $coursestatus = 'Old';
            ...
            /**
             * Response return to json is summaryimage and summary added as parameter(modified) - Kesavamoorthy - 20/02/2013
             */
            $context = context_course::instance($course->id);
            $summaryimage = file_rewrite_pluginfile_urls($course->summary, 'pluginfile.php', $context->id, 'course', 'summary', NULL);
			$coursepreviewimage = file_rewrite_pluginfile_urls($course->coursepreview, 'pluginfile.php', $context->id, 'course', 'coursepreview', NULL);
            $result[] = array('id'=>$course->id, 'shortname'=>$course->shortname, 'fullname'=>$course->fullname, 'idnumber'=>$course->idnumber,'visible'=>$course->visible, 'enrolledusercount'=>$enrolledusercount,'summary'=>$summaryimage,'coursearrival'=>$coursestatus, 'categoryid'=>$course->category, 'viewtype'=>$course->viewtype, 'coursepreview'=>$coursepreviewimage);
        }
        ...
        return new external_multiple_structure(
            new external_single_structure(
                array(
                    ...
                    'summary'   => new external_value(PARAM_RAW, 'summary'), //Summary is added in the service core_enrol_get_users_courses - Kesavamoorthy - 20/02/2013
                    'coursearrival' => new external_value(PARAM_RAW, 'coursearrival'), //Start date is added in the service core_enrol_get_users_courses - Kesavamoorthy - 21/02/2013
                    'categoryid' => new external_value(PARAM_RAW, 'categoryid'), //categoryid is added in the service core_enrol_get_users_courses - Kesavamoorthy - 25/02/2013
					'viewtype' => new external_value(PARAM_RAW, 'user view type. 0 means accrodion, 1 means tile'), 'coursepreview' => new external_value(PARAM_RAW, 'Course preview image'), // Added for video tile
     ...
     * @return external_function_parameters
     */
    public static function get_users_courses_subcat_parameters() {
        return new external_function_parameters(
            array(
                'userid' => new external_value(PARAM_INT, 'user id'),
                'categoryid' => new external_value(PARAM_INT, 'category id'),
     ...
     * @return array of courses
     */
    public static function get_users_courses_subcat_offline($userid, $categoryid, $cattype=NULL) {
        global $USER, $DB;
		...
		$courses = enrol_get_users_courses($params['userid'],$params['categoryid'],$params['cattype'], true, 'id, shortname, fullname, idnumber, visible, viewtype, coursepreview,timecreated,timemodified,startdate');
		if($list_of_subcategories) {
			foreach($list_of_subcategories as $kSubCat => $vSubCat) {
				$subcategory = $vSubCat->id;
				$courses = array_merge($courses,enrol_get_users_courses($params['userid'],$subcategory,$params['cattype'], true, 'id, shortname, fullname, idnumber, visible, viewtype, coursepreview,timecreated,timemodified,startdate'));
			}
		}
        $result = array();
        ...
        foreach ($courses as $course) {
           ...
           $aweekago = strtotime('-30 days');
            ...
            $summaryimage = file_rewrite_pluginfile_urls($course->summary, 'pluginfile.php', $context->id, 'course', 'summary', NULL);
			$coursepreviewimage = file_rewrite_pluginfile_urls($course->coursepreview, 'pluginfile.php', $context->id, 'course', 'coursepreview', NULL);
            $result[] = array('id'=>$course->id, 'shortname'=>$course->shortname, 'fullname'=>$course->fullname, 'idnumber'=>$course->idnumber,'visible'=>$course->visible, 'enrolledusercount'=>$enrolledusercount,'summary'=>$summaryimage,'coursearrival'=>$coursestatus, 'categoryid'=>$course->category, 'viewtype'=>$course->viewtype, 'coursepreview'=>$coursepreviewimage,'timecreated'=>$course->timecreated,'timemodified'=>$course->timemodified,'startdate'=>$course->startdate);
     ...
     * @return array of courses
     */
    public static function get_users_courses_subcat($userid, $categoryid, $cattype=NULL) {
        global $USER, $DB;
     ...
     * @return external_description
     */
    public static function get_users_courses_subcat_returns() {
        return new external_multiple_structure(
            new external_single_structure(
                array(
                    'id'        => new external_value(PARAM_INT, 'id of course'),
                    ...
                    'coursearrival' => new external_value(PARAM_RAW, 'coursearrival'), //Start date is added in the service core_enrol_get_users_courses - Kesavamoorthy - 21/02/2013
                    'categoryid' => new external_value(PARAM_RAW, 'categoryid'), //categoryid is added in the service core_enrol_get_users_courses - Kesavamoorthy - 25/02/2013
					'viewtype' => new external_value(PARAM_RAW, 'user view type. 0 means accrodion, 1 means tile'),
					'coursepreview' => new external_value(PARAM_RAW, 'Course preview image'), // Added for video tile
                )
            )
        );
    }
...
class moodle_enrol_external extends external_api {
     ...
     */
    public static function get_users_courses($userid,$categoryid) {
        //echo "check".$categoryid;
        return core_enrol_external::get_users_courses($userid,$categoryid);
    }
