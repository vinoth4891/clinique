<?php
	...
	if($rolename == 'subadmin') {
		...
		if($enrol_ids) {	
			$join = " LEFT JOIN {user_enrolments} ue ON ue.userid = u.id ";
			...
			$subAdminSql = " ue.enrolid IN ('".$enroll_id_list_str."')";
			$extrasql .= $subAdminSql;
			//var_dump($enroll_id_list);				
		} else {
			$extrasql .= " u.id = ".$roles[$role]->userid;
			$subAdminSql = " u.id = ".$roles[$role]->userid;
		}	
