<?php
    ...
    $returnurl = new moodle_url('/admin/user_subadmin.php', array('sort' => $sort, 'dir' => $dir, 'perpage' => $perpage, 'page'=>$page));
    ...
    foreach ($columns as $column) {
        ...
        $$column = "<a href=\"user_subadmin.php?sort=$column&amp;dir=$columndir\">".$string[$column]."</a>$columnicon";
    }
	...
	if($rolename == 'subadmin') {
		/** Fetch users who are assigned to the same cohorts as the subadmin assigned to
		 * 1. Fetch all cohorts that the logged-in subadmin manages from {countr_user} table
		 * 2. Filter {country_user}  table with the lang from #1
		 */
		$subadmin_cohorts = $get_all_cat = $DB->get_record('country_user', array('userid' => $USER->id));
		$join = " JOIN {cohort_members} cm ON u.id = cm.userid"
				. " JOIN {cohort} c ON c.id = cm.cohortid ";
		$subAdminSql = " c.idnumber IN ('" . implode("','",explode(',', $subadmin_cohorts->lang)) . "')";
		
		if($extrasql) {
			$extrasql .= " AND ";
		}

		$extrasql .= $subAdminSql;
		
		$users = get_users_listing_subadmin($sort, $dir, $page*$perpage, $perpage, '', '', '', $extrasql, $params, $context, $join);
		$usercount = get_users_subadmin(false, '', false, null, "", '', '', '', '', '*', $subAdminSql, $params, $join);
		$usersearchcount = get_users_subadmin(false, '', false, null, "", '', '', '', '', '*', $extrasql, $params, $join);
    ...
    $baseurl = new moodle_url('/admin/user_subadmin.php', array('sort' => $sort, 'dir' => $dir, 'perpage' => $perpage));
