...
<script type="text/javascript">
	$(document).ready(function() {
		$('#id_previewrows').val('100');
		$('#id_encoding, #id_previewrows, #id_delimiter_name').prop('disabled', true);
	});
</script>
...
if ($formdata = $mform2->is_cancelled()) {
    ...
    while ($line = $cir->next()) {
        ...
        foreach ($STD_FIELDS as $field) {
			if($field === 'city' && empty($user->city) && !$existinguser) {
				$user->$field = 'Unknown';
			}			
            if (isset($user->$field)) {
                continue;
...
var UserUpload = {
	FORCE_PASSWORD_NONE : 0,
	FORCE_PASSWORD_CHANGE_WEAK_PASSWORD : 1,
	FORCE_PASSWORD_ALL : 2,
	YES : 1,
	NO : 0 
};
var rolename = '<?php echo $rolename;?>'; 	

//Give the IDs of the fields to be hid for Subadmin
var subadminHiddenFields = ['defaultheader','category_1','fitem_id_uuforcepasswordchange','fitem_id_uustandardusernames','fitem_id_uuallowrenames','fitem_id_uuallowdeletes','fitem_id_uuallowsuspends','fitem_id_uunoemailduplicates','fitem_id_uubulk'];
	$(document).ready(function() {
		if($('#id_city').length) { //City header is NOT available in CSV
			$('#fitem_id_city').hide();
		}

		if(rolename == 'subadmin'){
			$('#id_uuforcepasswordchange').val(UserUpload.YES);
			$('#id_uustandardusernames').val(UserUpload.NO);
			$('#id_uutype option[value="1"]').remove();

			$.each(subadminHiddenFields,function(index, value) {
				$('#' + value).hide();
			});

			$('#mform2').submit(function(){
				$('#id_uuallowrenames').val(UserUpload.NO);	
				$('#id_uuallowdeletes').val(UserUpload.NO);	
				$('#id_uuallowsuspends').val(UserUpload.NO);	
				$('#id_uunoemailduplicates').val(UserUpload.NO);	
			});
		}
	});
</script>
