...
require_once($CFG->dirroot.'/cohort/locallib.php'); // Added for member add to cohort
...
if ($usernew = $userform->get_data()) {
    ...
    if ($user->id == $USER->id) {
		...
		if($rolename == 'subadmin') {
			redirect("$CFG->wwwroot/$CFG->admin/user_subadmin.php");
		} else {
...
if($rolename == 'subadmin') {
	$user_country = $DB->get_record('country_user', array('userid' => $USER->id));
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		if(rolename == 'subadmin'){
			$('#fitem_id_maildisplay').hide();
			$('#fitem_id_mailformat').hide();
			$('#fitem_id_maildigest').hide();
			$('#fitem_id_autosubscribe').hide();
			$('#fitem_id_trackforums').hide();
			$('#fitem_id_htmleditor').hide();
			$('#fitem_id_theme').hide();
			$('#fitem_id_description_editor').hide();
			$('#moodle_interests').hide();
			$('#moodle_picture').hide();
			$('#moodle_optional').hide();
			$('#fitem_id_suspended').hide();
			$('#fitem_id_auth').hide();
			$('#fitem_id_timezone').hide();
		}
		<?php if($sUserAssignLang) { ?>
		var lang_val = <?php echo $sUserAssignLang;?>;
		var selected = '<?php echo $user->lang; ?>';
		<?php } else { ?>
		var lang_val = '';
		<?php } ?>
		if(lang_val) {
			...
			$.each(lang_val, function(i,e){
				 $("#id_lang").append("<option value='" + i + "'>" + e + "</option>");
			});
			$("#id_lang option[value='" + selected + "']").attr('selected', true);
		}
	});
</script>
<?php
