...
class profile_field_menu extends profile_field_base {
    ...
    function profile_field_menu($fieldid=0, $userid=0) {
		...
		$options = array_map(function($region) {
			return strtolower(trim($region));	
		}, $this->options);
        /// Set the data key
        if ($this->data !== NULL) {
            $this->datakey = (int)array_search(strtolower(trim($this->data)), $options);
        }
    }
    ...
    function edit_field_add($mform) {
        $mform->addElement('select', $this->inputname, format_string(get_string($this->inputname, 'admin')), $this->options);
    }
    ...
    function convert_external_data($value) {
        ...
        $retval = array_search(strtolower(trim($value)), $options);
