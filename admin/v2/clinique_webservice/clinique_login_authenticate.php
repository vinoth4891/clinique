...
class Login {
    ...
    public function __authenticate($username, $password, $serviceshortname) {
		...
		if( ! empty($user->profile['region'])) {
            if(strstr($user->profile['region'], '/')){
						$region_name = explode('/', strtolower($user->profile['region']));
						$user->profile['region'] = implode('/', array_map('ucwords',$region_name));
                    }else{
                        $user->profile['region'] =  ucwords(strtolower($user->profile['region']));
              }
           if(strstr($user->profile['store'], '/')){
						$store_name = explode('/', strtolower($user->profile['store']));
						$user->profile['store'] = implode('/', array_map('ucwords',$store_name));
                    }else{
                        $user->profile['store'] =  ucwords(strtolower($user->profile['store']));
              }
           if(strstr($user->profile['retailer'], '/')){
						$retailer_name = explode('/', strtolower($user->profile['retailer']));
						$user->profile['retailer'] = implode('/', array_map('ucwords',$retailer_name));
                    }else{
                        $user->profile['retailer'] =  ucwords(strtolower($user->profile['retailer']));
              }
		}
...
// log token access
			...
			$user->role = '';
			if(self::__is_report_user($user->id)) {
            ...
            $user->country = self::getCountryByCode($user->country);
	...
	public static function __is_report_user($userid) {
		global $DB;
		$rolesql = "SELECT id
					  FROM {role} 
					 WHERE shortname = ?";
		$roleid = array_values($DB->get_records_sql($rolesql, array('reportuser')));
		$reportuser= array_values($DB->get_records_sql("SELECT id FROM {role_assignments} WHERE roleid=".$roleid[0]->id." AND userid=".$userid.""));
		return ! empty($reportuser[0]->id);
	}
